...
public class AdminController {
    ...
    public ResponseEntity<Map<String, Object>> getGlobalStats() {
        ...
        long totalSpaces = ((Number) entityManager.createNativeQuery("SELECT COUNT(*) FROM spaces").getSingleResult()).longValue();
        long surveyedSpaces = ((Number) entityManager.createNativeQuery("SELECT COUNT(*) FROM survey_responses WHERE status = 'SUBMITTED'").getSingleResult()).longValue();
        long draftSurveys = ((Number) entityManager.createNativeQuery("SELECT COUNT(*) FROM survey_responses WHERE status = 'DRAFT'").getSingleResult()).longValue();
        
        stats.put("totalSpaces", totalSpaces);
        stats.put("surveyedSpaces", surveyedSpaces);
        stats.put("draftSurveys", draftSurveys);
        stats.put("completionRate", totalSpaces > 0 ? (double) surveyedSpaces / totalSpaces * 100 : 0);
    ...
    @Transactional(readOnly = true)
    public ResponseEntity<?> getPropertyFloors(@PathVariable("id") Long id) {
        try {
    ...
    @Transactional(readOnly = true)
    public ResponseEntity<?> getFloorSpaces(@PathVariable("floorId") Long floorId) {
        try {
            List<Object[]> results = entityManager.createNativeQuery(
