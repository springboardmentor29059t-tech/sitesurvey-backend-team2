...
public class SpaceController {
    ...
    @PostMapping
    @Transactional
    public ResponseEntity<?> createSpace(@RequestBody Map<String, Object> request) {
        try {
            Long floorId = Long.valueOf(request.get("floorId").toString());
            String name = (String) request.get("name");
            String type = (String) request.get("type");
            String geometry = (String) request.get("geometry");

            entityManager.createNativeQuery("INSERT INTO spaces (floor_id, name, type, geometry_wkt, created_at) VALUES (:fid, :name, :type, :geom, NOW())")
                .setParameter("fid", floorId)
                .setParameter("name", name)
                .setParameter("type", type)
                .setParameter("geom", geometry)
                .executeUpdate();

            return ResponseEntity.ok(Map.of("success", true, "message", "Space created"));
        } catch (Exception e) {
    ...
    @Transactional
    public ResponseEntity<?> clearPropertySpaces(@PathVariable("propertyId") Long propertyId) {
        try {
